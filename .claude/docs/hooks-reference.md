...
12 个 Hook 仅在 Claude Code 平台上原生支持，其他平台不支持 Hook 机制。

| 平台 | Hook 支持 | 替代方案 |
| --- | --- | --- |
| Claude Code | 原生支持（12 个 Hook） | — |
| Codex | 不支持 | AGENTS.md 文档指令 |
| Cursor | 不支持 | AGENTS.md 文档指令 |
| Windsurf | 不支持 | AGENTS.md 文档指令 |
| Trae IDE | 不支持 | AGENTS.md 文档指令 |
| Hermes Agent | 部分支持（后台审视机制） | SOUL.md 身份指令 |
| WorkBuddy | 不支持 | MCP 协议替代 |

### Hook 功能降级映射

在非 Claude Code 平台上，Hook 的自动化功能降级为 AGENTS.md 中的文档指令：

| Hook | 降级为 AGENTS.md 指令 |
| --- | --- |
| `validate-commit.sh` | "提交信息必须遵循 Conventional Commits 格式" |
| `validate-push.sh` | "禁止向 main/master/release/* 分支直接推送" |
| `session-start.sh` | "会话开始时先读取 active.md 了解当前状态" |
| `pre-compact.sh` | "上下文压缩前将进度写入 active.md" |
| `post-compact.sh` | "上下文压缩后从 active.md 恢复状态" |
| `validate-assets.sh` | "文件命名遵循 kebab-case，JSON/YAML 文件需格式合法" |
| `validate-skill-change.sh` | "技能变更后需运行 /skill-test 验证" |
| `detect-gaps.sh` | "会话开始时检查项目文档和配置缺口" |
| `notify.sh` | 无需降级（仅通知功能） |
| `session-stop.sh` | "会话结束时将 active.md 归档到 session-logs/" |
| `log-agent.sh` | "子 Agent 启动时记录审计日志" |
| `log-agent-stop.sh` | "子 Agent 完成时记录审计日志" |

### 规范源

Hook 脚本源码位于 `.studio/hooks/` 目录，通过同步脚本输出到 `.claude/hooks/`。Hook 的平台兼容性说明见 `.studio/hooks/_manifest.md`。